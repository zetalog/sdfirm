...
static struct imsic_data _imsic_data;
...
int imsic_sbi_init_cold(void)
{
	...
	int nodeoff = fdt_node_offset_by_compatible(fdt, 0,
                                                irqchip_imsic_match[0].compatible);
	return irqchip_imsic_cold_init(fdt, nodeoff, fdt_irqchip_imsic.match_table);
}
