...
static int pmu_add_hw_event_map(uint32_t eidx_start, uint32_t eidx_end, uint32_t cmap,
...
#if __riscv_xlen == 32
	mhpmevent_csr = CSR_MHPMEVENTH(3)  + ctr_idx - 3;
...
static int pmu_ctr_start_hw(uint32_t cidx, uint64_t ival, bool ival_update)
{
	...
	__clear_bit(cidx, (volatile bits_t *)&mctr_inhbt);
...
static int pmu_ctr_stop_hw(uint32_t cidx)
{
	...
	if (!__test_bit(cidx, &mctr_inhbt)) {
		__set_bit(cidx, (volatile bits_t *)&mctr_inhbt);
		csr_write(CSR_MCOUNTINHIBIT, mctr_inhbt);
		return 0;
	} else
...
static int pmu_reset_hw_mhpmevent(int ctr_idx)
{
	...
	if (sbi_hart_has_extension(sbi_scratch_thishart_ptr(),
		...
		csr_write_num(CSR_MHPMEVENTH(3) + ctr_idx - 3, 0);
#else
	csr_write_num(CSR_MHPMEVENT3 + ctr_idx - 3, 0);
#endif
				   ...
				   uint64_t data)
{
	...
	if (sbi_hart_has_extension(scratch, SBI_HART_EXT_SSCOFPMF))
		csr_write_num(CSR_MHPMEVENTH(3) + ctr_idx - 3,
 ...
 * check.
 */
static int pmu_ctr_find_fw(unsigned long cbase, unsigned long cmask, uint32_t hartid)
{
	int i = 0;
	int fw_base;
	unsigned long ctr_mask = cmask << cbase;
...
static void pmu_reset_event_map(uint32_t hartid)
{
	int j;
