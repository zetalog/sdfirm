...
void pci_find_capability_list(int index, int bus, int dev, int fun)
{
	...
	while ((ttl)--) {
		...
		if (id == 0xff)
			break;
		printf("\tBDF (%d-%d-%d) cap %s is found at 0x%x\n", bus, dev, fun, cap_list[id - 1], pos);
		pos = (ent >> 8);
	}
}

uint16_t pci_find_ext_capability(int index, int bus, int dev, int fun, int cap)
{
	uint32_t header;
	int ttl;
...
void pci_find_ext_capability_list(int index, int bus, int dev, int fun)
{
	...
	while (ttl-- > 0) {
		...
		printf("\tBDF (%d-%d-%d) Extended cap %s is found at 0x%x\n",
			bus, dev, fun, cap_ext_list[PCI_EXT_CAP_ID(header) - 1], pos);
