...
serial_port uart_open(const char *pcPortName)
{
	...
	sp->hPort = CreateFileA(acPortName,
...
{
	PurgeComm(((serial_port_windows *)sp)->hPort,
...
void uart_set_speed(serial_port sp, const uint32_t uiPortSpeed)
{
	...
	log_put(LOG_CATEGORY, NFC_PRIORITY_TRACE,
		"Serial port speed requested to be set to %d bauds.",
		uiPortSpeed);
	...
	if (!SetCommState(spw->hPort, &spw->dcb)) {
		log_put(LOG_CATEGORY, NFC_PRIORITY_ERROR,
			"%s", "Unable to apply new speed settings.");
		return;
	}
...
int uart_receive(serial_port sp, byte_t *pbtRx, const size_t szRx,
		 void *abort_p, struct timeval *timeout)
{
	DWORD dwBytesToGet = (DWORD)szRx;
	DWORD dwBytesReceived = 0;
	DWORD dwTotalBytesReceived = 0;
	BOOL res;
	volatile bool * abort_flag_p;
	...
	log_put(LOG_CATEGORY, NFC_PRIORITY_TRACE,
		"Timeouts are set to %u ms", timeout_ms);
	
	/* TODO Enhance the reception method
	 * - According to MSDN, it could be better to implement
	 * nfc_abort_command() mecanism using Cancello()
	 */
	abort_flag_p = (volatile bool *)abort_p;
	do {
		log_put (LOG_CATEGORY, NFC_PRIORITY_TRACE, "ReadFile");
		res = ReadFile(((serial_port_windows *) sp)->hPort,
			       pbtRx + dwTotalBytesReceived,
			       dwBytesToGet,
		...
		if (abort_flag_p != NULL &&
		    (*abort_flag_p) && dwTotalBytesReceived == 0) {
			return EOPABORT;
		}
	      ...
	      struct timeval *timeout)
{
	...
	if (!WriteFile(((serial_port_windows *)sp)->hPort,
		       pbtTx, szTx, &dwTxLen, NULL)) {
		return ECOMIO;
	}
	if (!dwTxLen)
		return ECOMIO;
...
/* Path to the serial port is OS-dependant. Try to guess what we should
 * use.
 */
...
char **uart_list_ports(void)
{
	...
	for (i = 1; i <= MAX_SERIAL_PORT_WIN; i++) {
		if (is_port_available(i)) {
			...
			printf("nfc_uart: found candidate port: %s\n",
