...
/* Provide internal function to manipulate nfc_device_t type */
...
nfc_device_t *nfc_device_new(void)
{
	 ...
	 * NOTE Actually, these initialization will be overwritten while
	 *      the device will be setup. Putting them to _false_ while
	 *      the default is _true_ ensure we send the command to the
	 *      chip
	 */
	res->bCrc = false;
	res->bPar = false;
