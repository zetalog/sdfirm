...
bool pn53x_usb_get_usb_device_name(struct usb_device *dev,
				   libusb_device_handle *udev,
				   char *buffer, size_t len);
bool pn53x_usb_init(nfc_device_t *pnd);

int pn53x_usb_bulk_read(struct pn53x_usb_data *data,
			byte_t abtRx[], const size_t szRx,
			struct timeval *timeout)
{
	...
	res = usb_bulk_read(data->pudh, data->uiEndPointIn,
			    (char *)abtRx, szRx, timeout_ms);
	if (res > 0) {
...
int pn53x_usb_bulk_write(struct pn53x_usb_data *data,
			 byte_t abtTx[], const size_t szTx,
			 struct timeval *timeout)
{
	int res;
	int timeout_ms = USB_INFINITE_TIMEOUT;
	...
	if (timeout)
		timeout_ms = timeout->tv_sec * 1000 + timeout->tv_usec / 1000;
	res = usb_bulk_write(data->pudh, data->uiEndPointOut,
			     (char *)abtTx, szTx, timeout_ms);
	if (res > 0) {
		/* HACK This little hack is a well know problem of USB,
		 * see http://www.libusb.org/ticket/6 for more details
		 */
		if ((res % data->uiMaxPacketSize) == 0) {
			usb_bulk_write(data->pudh, data->uiEndPointOut,
				       "\0", 0, timeout_ms);
		}
	} else {
		log_put(LOG_CATEGORY, NFC_PRIORITY_ERROR,
...
const struct pn53x_usb_supported_device pn53x_usb_supported_devices[] = {
	{ 0x072F, 0x2200, NXP_PN531,   "ACS / ACR122U for NFC" },
...
pn53x_usb_model pn53x_usb_get_device_model(uint16_t vendor_id,
					   uint16_t product_id)
{
	size_t n;
	for (n = 0;
	     n < (sizeof (pn53x_usb_supported_devices) /
		  sizeof (struct pn53x_usb_supported_device));
	     n++) {
...
void pn53x_usb_get_end_points(struct usb_device *dev, struct pn53x_usb_data *data)
{
	uint32_t uiIndex;
	uint32_t uiEndPoint;
	struct libusb_interface_descriptor *puid;
	
	puid = dev->config->Interface->altsetting;
	
	/* 3 Endpoints maximum: Interrupt In, Bulk In, Bulk Out */
	for (uiIndex = 0; uiIndex < puid->bNumEndpoints; uiIndex++) {
		/* Only accept bulk transfer endpoints (ignore interrupt
		 * endpoints)
		 */
		if (puid->endpoint[uiIndex].bmAttributes != LIBUSB_TRANSFER_TYPE_BULK)
			continue;
		/* Copy the endpoint to a local var, makes it more
		 * readable code
		 */
	...
	for (i = 0; i < nr_devs; i++) {
		...
		for (n = 0;
			...
			if ((pn53x_usb_supported_devices[n].vendor_id == dev->descriptor.idVendor) &&
				...
				/* Make sure there are 2 endpoints
				 * available with libusb-win32 we
				 * got some null pointers so be
				 * robust before looking at
				 * endpoints:
				 */
				if (dev->config == NULL ||
				    dev->config->Interface == NULL ||
				    dev->config->Interface->altsetting == NULL) {
					/* Nope, we maybe want the
					 * next one, let's try to
					 * find another
					 */
					continue;
				}
				...
				udev = usb_open(dev);
				/* Set configuration */
				res = libusb_set_configuration(udev, 1);
				if (res < 0) {
					log_put(LOG_CATEGORY, NFC_PRIORITY_ERROR,
				...
				pn53x_usb_get_usb_device_name(dev, udev,
							      pnddDevices[*pszDeviceFound].acDevice,
							      sizeof (pnddDevices[*pszDeviceFound].acDevice));
				usb_close(udev);
				pnddDevices[*pszDeviceFound].pcDriver = PN53X_USB_DRIVER_NAME;
				pnddDevices[*pszDeviceFound].uiBusIndex = -1;
				(*pszDeviceFound)++;
...
nfc_device_t *pn53x_usb_connect(const nfc_device_desc_t *pndd)
{
	...
	for (i = 0; i < nr_devs; i++) {
		...
		/* Set configuration */
		res = libusb_set_configuration(data.pudh, 1);
		if (res < 0) {
			log_put(LOG_CATEGORY, NFC_PRIORITY_ERROR,
				"Unable to set USB configuration (%s)",
				_usb_strerror(res));
			if (EPERM == -res) {
				log_put(LOG_CATEGORY, NFC_PRIORITY_WARN,
					"Please double check USB permissions for device %04x:%04x",
		...
		data.model = pn53x_usb_get_device_model(dev->descriptor.idVendor,
							dev->descriptor.idProduct);
		/* Allocate memory for the device info and
		 * specification, fill it and return the
		 * info.
		 */
		pnd = nfc_device_new();
		pn53x_usb_get_usb_device_name(dev, data.pudh,
					      pnd->acName,
					      sizeof (pnd->acName));
		...
		*DRIVER_DATA(pnd) = data;
		...
		/* HACK1 Send first an ACK as Abort
		 ...
		 *       between host & device in case
		 *       host used set_configuration and
		 *       expects the device to have reset
		 *       its toggle bit, which PN53x
		 *       doesn't do.
		 */
...
void pn53x_usb_disconnect(nfc_device_t *pnd)
{
	...
	if (DRIVER_DATA(pnd)->model == ASK_LOGO) {
		/* Set P30, P31, P32, P33, P35 to logic 1 and P34 to 0
		 * logic.
		 */
		/* ie. Switch all LEDs off and turn off progressive field.
		 */
		pn53x_write_register(pnd, PN53X_SFR_P3, 0xFF,
				     _BV(P30) | _BV(P31) | _BV(P32) | _BV(P33) | _BV(P35));
	}
	...
	if ((res = usb_close(DRIVER_DATA (pnd)->pudh)) < 0) {
		log_put(LOG_CATEGORY, NFC_PRIORITY_ERROR,
			"Unable to close USB connection (%s)",
			_usb_strerror(res));
	}
	pn53x_data_free(pnd);
	nfc_device_free(pnd);
...
#define PN53X_USB_BUFFER_LEN	(PN53x_EXTENDED_FRAME__DATA_MAX_LEN + \
		    ...
		    struct timeval *timeout)
{
	...
	byte_t  abtFrame[PN53X_USB_BUFFER_LEN] = { 0x00, 0x00, 0xff };
	size_t szFrame = 0;
	int res;
	byte_t abtRxBuf[PN53X_USB_BUFFER_LEN];
	
	pn53x_build_frame(abtFrame, &szFrame, pbtData, szData);
	res = pn53x_usb_bulk_write(DRIVER_DATA(pnd), abtFrame, szFrame, timeout);
	if (res < 0) {
		pnd->iLastError = ECOMIO;
		return false;
	}
	res = pn53x_usb_bulk_read(DRIVER_DATA(pnd),
				  abtRxBuf, sizeof (abtRxBuf), timeout);
	if (res < 0) {
		pnd->iLastError = ECOMIO;
	...
	if (pn53x_check_ack_frame(pnd, abtRxBuf, res)) {
		...
		/* For some reasons (eg. send another command while a
		 * previous one is running), the PN533 sometimes directly
		 * replies the response packet instead of ACK frame, so we
		 * send a NACK frame to force PN533 to resend response
		 * packet. With this hack, the nextly executed function
		 * (ie. pn53x_usb_receive()) will be able to retreive the
...
int pn53x_usb_receive(nfc_device_t *pnd,
		      byte_t *pbtData, const size_t szDataLen,
		      struct timeval *timeout)
{
	byte_t btDCS;
	size_t len;
	...
	/* If no timeout is specified but the command is blocking, force
	 * a 250ms timeout to allow breaking the loop if the user wants to
	 * stop it.
	 */
...
int pn53x_usb_ack(nfc_device_t *pnd)
{
	return pn53x_usb_bulk_write(DRIVER_DATA(pnd),
				    (byte_t *)pn53x_ack_frame,
...
bool pn53x_usb_init(nfc_device_t *pnd)
{
	...
	/* Sometimes PN53x USB doesn't reply ACK one the first frame, so
	 * we need to send a dummy one...
	 */
#if 0
	/* Sony RC-S360 doesn't support this command for now so let's use
	 * a get_firmware_version instead.
	 */
	...
	if (SONY_RCS360 == DRIVER_DATA (pnd)->model) {
		...
		log_put(LOG_CATEGORY, NFC_PRIORITY_TRACE,
			"SONY RC-S360 initialization.");
		pn53x_transceive(pnd, abtCmd2, sizeof (abtCmd2),
				 NULL, NULL, NULL);
		pn53x_usb_ack(pnd);
	}
	if (!pn53x_init(pnd))
	...
	if (ASK_LOGO == DRIVER_DATA(pnd)->model) {
		 ...
		 */
		pn53x_write_register(pnd, PN53X_REG_Control_switch_rng,
				     0xFF,
				     SYMBOL_CURLIMOFF | SYMBOL_SIC_SWITCH_EN |
				     SYMBOL_RANDOM_DATAREADY);
		 ...
		 *   * LED 1: bit 2 (P32)
		 *   * LED 2: bit 1 (P31)
		 *   * LED 3: bit 0 or 3 (depending of hardware revision)
		 *            (P30 or P33)
		 *   * LED 4: bit 5 (P35)
		 *  Notes: 
		 *   * Set logical 0 to switch LED on; logical 1 to switch
		 *     LED off. 
		 *   * Bit 4 should be maintained at 1 to keep RF field
		 *     on.
		 *  Progressive field activation:
		 *   The ASK LoGO hardware can progressively power-up the
		 *   antenna.  To use this feature we have to switch on
		 *   the field by switching on the field on PN533
		 *   (RFConfiguration) then set P34 to '1', and cut-off
		 *   the field by switching off the field on PN533 then
		 *   set P34 to '0'.
		 */
		/* Set P30, P31, P33, P35 to logic 1 and P32, P34 to 0
		 * logic.
		 */
		/* ie. Switch LED1 on and turn off progressive field */
		pn53x_write_register(pnd, PN53X_SFR_P3, 0xFF,
				     _BV(P30) | _BV(P31) | _BV(P33) | _BV(P35));
	}
	return true;
}

bool pn53x_usb_configure(nfc_device_t *pnd,
			 ...
			 const bool bEnable)
{
	...
	case ASK_LOGO:
		if (NDO_ACTIVATE_FIELD == ndo) {
			/* Switch on/off LED2 and Progressive Field GPIO
			 * according to ACTIVATE_FIELD option.
			 */
			log_put(LOG_CATEGORY, NFC_PRIORITY_TRACE,
				"Switch progressive field %s",
				bEnable ? "On" : "Off");
			if (!pn53x_write_register(pnd, PN53X_SFR_P3,
						  (uint8_t)(_BV(P31) | _BV(P34)),
						  (uint8_t)(bEnable ? _BV(P34) : _BV(P31))))
				return false;
		}
		break;
	case SCM_SCL3711:
		if (NDO_ACTIVATE_FIELD == ndo) {
			/* Switch on/off LED according to ACTIVATE_FIELD
			 * option.
			 */
			if (!pn53x_write_register(pnd, PN53X_SFR_P3,
						  _BV(P32),
						  (uint8_t)(bEnable ? 0 : _BV(P32))))
				return false;
		}
		break;
	default:
...
bool pn53x_usb_abort_command(nfc_device_t * pnd)
{
