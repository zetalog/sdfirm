...
static struct linux_device_handle_priv *__device_handle_priv(struct libusb_device_handle *handle)
{
				       ...
				       unsigned char *buffer)
{
	...
	r = read(fd, buffer, LIBUSB_DT_DEVICE_SIZE);;
	close(fd);
	if (r < 0) {
		usbi_err(DEVICE_CTX(dev), "read failed, ret=%d errno=%d", fd, errno);
		return LIBUSB_ERROR_IO;
	} else if (r < LIBUSB_DT_DEVICE_SIZE) {
		usbi_err(DEVICE_CTX(dev), "short read %d/%d", r, LIBUSB_DT_DEVICE_SIZE);
		return LIBUSB_ERROR_IO;
	}
					      ...
					      unsigned char *buffer, size_t len)
{
	...
	if (off < 1) {
		...
		close(fd);
		return LIBUSB_ERROR_IO;
	} else if (off == LIBUSB_DT_DEVICE_SIZE) {
		close(fd);
		return LIBUSB_ERROR_NOT_FOUND;
	}

	off = lseek(fd, LIBUSB_DT_DEVICE_SIZE, SEEK_SET);
	if (off < 0) {
			     ...
			     uint8_t devaddr, const char *sysfs_dir)
{
	...
	if (sysfs_can_relate_devices) {
		int tmp = sysfs_get_active_config(dev, (uint8_t *)&active_config);
		if (tmp < 0)
			return tmp;
		if (active_config == (uint8_t)-1)
			device_configured = 0;
	...
	dev_buf = malloc(LIBUSB_DT_DEVICE_SIZE);
	if (!dev_buf) {
		close(fd);
		return LIBUSB_ERROR_NO_MEM;
	}

	r = read(fd, dev_buf, LIBUSB_DT_DEVICE_SIZE);
	if (r < 0) {
		usbi_err(DEVICE_CTX(dev),
	...
	dev->num_configurations = dev_buf[LIBUSB_DT_DEVICE_SIZE - 1];
			     ...
			     int *usbfs_fallback)
{
	...
	return enumerate_device(ctx, _discdevs, (uint8_t)(busnum & 0xff),
				(uint8_t)(devaddr & 0xff), devname);
}
...
static int op_set_configuration(struct libusb_device_handle *handle,
				uint8_t config)
{
	struct linux_device_priv *priv = __device_priv(handle->dev);
	int fd = __device_handle_priv(handle)->fd;
	int r = ioctl(fd, IOCTL_USBFS_SETCONFIG, (unsigned long)&config);
	if (r) {
		if (errno == EINVAL)
			return LIBUSB_ERROR_NOT_FOUND;
		else if (errno == EBUSY)
...
{
	int fd = __device_handle_priv(handle)->fd;
	int r = ioctl(fd, IOCTL_USBFS_CLAIMINTF, (unsigned long)&iface);
	if (r) {
		if (errno == ENOENT)
			return LIBUSB_ERROR_NOT_FOUND;
		else if (errno == EBUSY)
...
{
	int fd = __device_handle_priv(handle)->fd;
	int r = ioctl(fd, IOCTL_USBFS_RELEASEINTF, (unsigned long)&iface);
	if (r) {
		if (errno == ENODEV)
			return LIBUSB_ERROR_NO_DEVICE;
			 ...
			 unsigned char endpoint)
{
	...
	int r = ioctl(fd, IOCTL_USBFS_CLEAR_HALT, (unsigned long)&_endpoint);
	if (r) {
		if (errno == ENOENT)
			return LIBUSB_ERROR_NOT_FOUND;
		else if (errno == ENODEV)
...
{
	int fd = __device_handle_priv(handle)->fd;
	int r = ioctl(fd, IOCTL_USBFS_RESET, (unsigned long)NULL);
	if (r) {
		if (errno == ENODEV)
			return LIBUSB_ERROR_NOT_FOUND;
				   ...
				   int Interface)
{
	...
	r = ioctl(fd, IOCTL_USBFS_IOCTL, (unsigned long)&command);
	if (r) {
		if (errno == ENODATA)
			return LIBUSB_ERROR_NOT_FOUND;
	...
	unsigned char urb_type)
{
	...
	num_urbs = transfer->length / MAX_BULK_BUFFER_LENGTH;
	last_urb_partial = 0;
	...
	for (i = 0; i < num_urbs; i++) {
		...
		r = ioctl(dpriv->fd, IOCTL_USBFS_SUBMITURB, (unsigned long)urb);
		if (r < 0) {
			int j;
			...
			for (j = 0; j < i; j++) {
				int tmp = ioctl(dpriv->fd, IOCTL_USBFS_DISCARDURB, (unsigned long)&urbs[j]);
				if (tmp && errno != EINVAL)
...
static int submit_iso_transfer(struct usbi_transfer *itransfer)
{
	...
	/* submit URBs */
	for (i = 0; i < num_urbs; i++) {
		int r = ioctl(dpriv->fd, IOCTL_USBFS_SUBMITURB, (unsigned long)urbs[i]);
		if (r < 0) {
			int j;
...
static int cancel_control_transfer(struct usbi_transfer *itransfer)
{
	...
	r = ioctl(dpriv->fd, IOCTL_USBFS_DISCARDURB, (unsigned long)tpriv->urbs);
	if(r) {
		if (errno == EINVAL) {
...
static int cancel_bulk_transfer(struct usbi_transfer *itransfer)
{
	...
	tpriv->reap_action = CANCELLED;
	for (i = 0; i < tpriv->num_urbs; i++) {
		int tmp = ioctl(dpriv->fd, IOCTL_USBFS_DISCARDURB, (unsigned long)&tpriv->urbs[i]);
		if (tmp && errno != EINVAL)
			usbi_warn(TRANSFER_CTX(transfer),
				"unrecognised discard errno %d", errno);
	}
...
static int cancel_iso_transfer(struct usbi_transfer *itransfer)
{
	...
	tpriv->reap_action = CANCELLED;
	for (i = 0; i < tpriv->num_urbs; i++) {
		int tmp = ioctl(dpriv->fd, IOCTL_USBFS_DISCARDURB,
				(unsigned long)tpriv->iso_urbs[i]);
		if (tmp && errno != EINVAL) {
			usbi_warn(TRANSFER_CTX(transfer),
				"unrecognised discard errno %d", errno);
		}
				  ...
				  struct usbfs_urb *urb)
{
	...
	if (urb_idx == num_urbs - 1) {
		...
		for (i = urb_idx + 1; i < tpriv->num_urbs; i++) {
			int r = ioctl(dpriv->fd, IOCTL_USBFS_DISCARDURB,
...
static int reap_for_handle(struct libusb_device_handle *handle)
{
	...
	r = ioctl(hpriv->fd, IOCTL_USBFS_REAPURBNDELAY, (unsigned long)&urb);
	if (r == -1 && errno == EAGAIN)
		return 1;
	if (r < 0) {
		if (errno == ENODEV)
			    ...
			    int num_ready)
{
	...
	for (i = 0; i < nfds && num_ready > 0; i++) {
		...
		num_ready--;
		list_for_each_entry(struct libusb_device_handle,
				    handle, &ctx->open_devs, list) {
