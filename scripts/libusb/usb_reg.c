  ...
  if(usb_registry_is_nt())
  ...
  else /* Win9x */
    {
      ...
      if(reg_key == INVALID_HANDLE_VALUE)
        {
          usbi_dbg("usb_registry_get_property(): reading "
                    "registry key failed");
          return FALSE;
        }
                                 ...
                                 char *buf, int size)
{
  ...
  if(usb_registry_is_nt())
      ...
      if(size > 2)
        {
          if(!SetupDiSetDeviceRegistryProperty(dev_info, dev_info_data,
                                               which, (BYTE *)buf, size))
            {
              usbi_dbg("usb_registry_set_property(): setting "
      ...
      else
        {
          if(!SetupDiSetDeviceRegistryProperty(dev_info, dev_info_data,
                                               which, NULL, 0))
            {
              usbi_dbg("usb_registry_set_property(): deleting "
                        "property '%s' failed", val_name);
              return FALSE;
            }
        }
  ...
  else
    {
      ...
      if(reg_key == INVALID_HANDLE_VALUE)
        {
          usbi_dbg("usb_registry_set_property(): reading "
                    "registry key failed");
          return FALSE;
        }
...
bool_t usb_registry_insert_class_filter(void)
{
  ...
  if(!keys)
    {
      usbi_dbg("usb_registry_insert_filter: unable to retrieve class keys\n");
      return FALSE;
    }
  ...
  while(key)
    {
      ...
      if(!usb_registry_set_mz_value(key->name, "UpperFilters", buf, 
        ...
        {
          usbi_dbg("usb_registry_insert_filter: unable to "
                    "set registry value\n");
        }
...
bool_t usb_registry_remove_class_filter(void)
{
  ...
  if(!keys)
    {
      usbi_dbg("usb_registry_remove_filter: unable to retrieve class keys\n");
      return FALSE;
    }
...
bool_t usb_registry_remove_device_filter(void)
{
  ...
  if(dev_info == INVALID_HANDLE_VALUE)
    {
      usbi_dbg("usb_registry_remove_device_filter(): getting "
                                            ...
                                            SP_DEVINFO_DATA *dev_info_data)
{
  ...
  if(!SetupDiSetClassInstallParams(dev_info, dev_info_data,
    ...
    {
      usbi_dbg("usb_registry_set_device_state(): setting class "
  ...
  if(!SetupDiCallClassInstaller(DIF_PROPERTYCHANGE, dev_info, dev_info_data))
    {
      usbi_dbg("usb_registry_set_device_state(): calling class "
                "installer failed");
      return FALSE;
    }
...
void usb_registry_stop_libusb_devices(void)
{
  ...
  if(dev_info == INVALID_HANDLE_VALUE)
    {
      usbi_dbg("usb_registry_stop_libusb_devices(): getting "
                "device info set failed");
      return;
    }
                          ...
                          SP_DEVINFO_DATA *dev_info_data)
{
  ...
  if(!usb_registry_get_property(SPDRP_HARDWAREID, dev_info, dev_info_data,
                                tmp, sizeof(tmp)))
    {
      usbi_dbg("usb_registry_match_no_hubs(): getting hardware id "
                "failed");
      return FALSE;
    }
...
bool_t usb_registry_restart_all_devices(void)
{
  ...
  if(dev_info == INVALID_HANDLE_VALUE)
    {
      usbi_dbg("usb_registry_restart_all_devices(): getting "
                "device info set failed");
      return FALSE;
    }
  ...
  while(SetupDiEnumDeviceInfo(dev_info, dev_index, &dev_info_data))
    {
      if(!usb_registry_get_property(SPDRP_HARDWAREID, dev_info, 
        ...
        {
          usbi_dbg("usb_registry_restart_all_devices(): getting hardware "
                    "id failed");
          dev_index++;
          continue;
        }
...
usb_class_key_t *usb_registry_get_usb_class_keys(void)
{
  ...
  if(dev_info == INVALID_HANDLE_VALUE)
    {
      usbi_dbg("usb_registry_get_class_keys(): getting "
                "device info set failed");
      return NULL;
    }
