...
#include <tmath.h>
#define exp tmath_expd
#include <target/page.h>
#include "genann_func.h"

extern struct genann_percpu *genann_ctx;
...
#ifdef CONFIG_GENANN_LOOKUP_SIZE
#define LOOKUP_SIZE CONFIG_GENANN_LOOKUP_SIZE
#else
...
#if CONFIG_GENANN_BUF_ONE_PAGE
	int page_cnt = (size + (PAGE_SIZE - 1))/PAGE_SIZE;
	if (page_cnt > 1) return 0;
	genann *ret = (genann *)page_alloc_pages(page_cnt);
#else
...
#ifdef CONFIG_GENANN_BUF_ONE_PAGE
	page_free_pages((struct page *)ann, 1); 
#else
    free(ann);
#endif
}
