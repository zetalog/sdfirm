...
static iommu_case_t case_s1_data;
...
static int s1_case_init(void)
{
    uint32_t ddi, pdi;
    if (case_s1_data.inited) {
        config_ddtp(case_s1_data.ddtp);
        tlb_inv_all();
        return 0;
    }
    // ppn_addr_set_tag("s1+s2 case ");
    // case_s1_data.ddtp.ppn = get_free_ppn(1, PPN_TYPE_DDT, "ddtp.ppm") ;
    case_s1_data.ddtp.ppn = get_free_ppn(1) ;
    ...
    case_s1_data.pdi_max = 1;
    // case_s1_data.data = (iommu_data_t*)(get_free_ppn(1, PPN_TYPE_OTHER, "iommu data")*PAGESIZE);
    case_s1_data.data = (iommu_data_t*)(get_free_ppn(1) * PAGESIZE);
    if (!case_s1_data.ddtp.ppn) {
        return -1;
    }
...
static int tbu_access_init(void)
{
    ...
    uint64_t iova = case_s1_data.data[0].iova;
    uint64_t iopa = case_s1_data.data[0].iopa;
...
int trl_s1_case(void)
{
    fail_if(s1_case_init() < 0);
