...
int ReadTraceByte(unsigned char *byte) {
	if (useMemoryInput) {
		if (traceIndex >= traceLength)
			return 0;
		*byte = __raw_readl(traceMem + traceIndex * 4);
		return 1;
	}
	else {
		//return fread(byte, 1, 1, fNex);
		return 0;
	}
}

void InitMemoryInput(caddr_t mem, size_t length) {
	traceMem = mem;
	traceLength = length;
	traceIndex = 0;
	useMemoryInput = 1;
...
int NexusDeco(FILE *f, int disp)
{
	...
	{
		prevByte = msgByte;
		if (!ReadTraceByte(&msgByte)) break;  // EOF
	...
	if (disp & 4)
	{
		...
		if (msgCnt > 0) printf(", %lx bytes/message", (msgBytes) / msgCnt);
		if (nInstr > 0) printf(", %d instr, %lx bits/instr", nInstr, (msgBytes * 8) / nInstr);
		printf("\n");
	}
...
DEFINE_COMMAND(nexrvdeco, do_nexrvdeco, "Nexus RISC-V Trace Decode commands",
	"cfg mem <addr> <length>\n"
